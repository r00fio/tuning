...
def stutiloutput(pipe):
        while True:
            try:
                ...
                traceback.print_exc()
proc = None
class StUtilRun(sublime_plugin.WindowCommand):
   
    def run(self):
        global proc
        path = expand_path(get_setting("workingdir", "/tmp", None), self.window)
        print path
        proc = subprocess.Popen('/usr/lib/tuning/stlink/st-util', cwd='/', stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print proc.stdout
        log_debug("Process: %s\n" % proc)
        t = threading.Thread(target=stutiloutput, args=(proc.stdout,))
        t.start()
...
class StUtilStop(sublime_plugin.WindowCommand):
    
    def run(self):
        global proc
        if proc == None:
            print "Seams like the st-util is already dead, but i'll try"
            p = subprocess.Popen("exec " + 'kill -9 $(pgrep st-util)', stdout=subprocess.PIPE, shell=True) 
        else:
            print "Kill st-util Process pid :" + str(proc.pid)
            proc.kill()
            # And FINISH HIM !
            p = subprocess.Popen("exec " + 'kill -9 $(pgrep st-util)', stdout=subprocess.PIPE, shell=True) 
